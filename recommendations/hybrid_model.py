...
def hybrid_recommendation(user_id, memory_matrix, similarity_matrix, user_ids, mf_model, candidate_asins,
                          w_memory=0.5, w_svd=0.5, n_recommendations=10):
    """
    ...
    Parameters:
      ...
      - candidate_asins: List of candidate product IDs to consider (e.g., top popular items).
      - w_memory: Weight for memory-based scores.
      - w_svd: Weight for SVD-based scores.
      - n_recommendations: Number of recommendations to return.

    Returns:
      A list of recommended product ASINs.
    """
    # Obtain scores from memory-based approach
    mem_scores = get_memory_based_scores(user_id, memory_matrix, similarity_matrix, user_ids)

    # Obtain scores from SVD (matrix factorization) approach
